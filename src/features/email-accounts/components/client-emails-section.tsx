import { FolderIcon, UserIcon, CalendarIcon } from "lucide-react"
...
import type { ClientEmailGroup, EmailServiceGroup } from "@/features/email-accounts/queries/get-email-accounts-by-client"
...
function ServiceBlock({
  ...
  svc: EmailServiceGroup
  serviceOptions: EmailServiceOption[]
  projectOptions: ProjectOption[]
  isFirst: boolean
}) {
  return (
    <div className={!isFirst ? "border-t" : undefined}>
      {/* Service subheader */}
...
function groupServicesByProject(
  ...
  const groups = new Map<string, ProjectServiceGroup>()

  for (const svc of services) {
    const key = svc.project_id ?? "__none__"
    if (!groups.has(key)) {
      groups.set(key, {
        project_id: svc.project_id ?? null,
        project_name: svc.project_id
          ? (projectMap.get(svc.project_id) ?? "Proyecto desconocido")
          : "Sin proyecto",
        services: [],
      })
    }
...
export function ClientEmailsSection({
  ...
  const hasMultipleProjects =
    new Set(group.services.map((s) => s.project_id ?? "__none__")).size > 1

  // If all services belong to a single project (or none), skip the project grouping layer
    ...
    return (
      <div className="overflow-hidden rounded-xl border bg-card">
        <div className="flex items-center gap-2 border-b bg-muted/40 px-5 py-3">
          ...
          <span className="text-xs text-muted-foreground">({totalAccounts} buzones)</span>
        </div>
  ...
  // Multiple projects: show one card per project
  const projectGroups = groupServicesByProject(group.services, projectOptions)

  return (
    <div className="space-y-4">
      {projectGroups.map((pg) => {
        const pgAccounts = pg.services.reduce((n, s) => n + s.accounts.length, 0)
        return (
          <div key={pg.project_id ?? "__none__"} className="overflow-hidden rounded-xl border bg-card">
            ...
            <div className="flex items-center gap-2 border-b bg-muted/50 px-5 py-3">
              ...
              <span className="text-xs text-muted-foreground">
                ({pgAccounts} buzone{pgAccounts !== 1 ? "s" : ""})
              </span>
            </div>

            {pg.services.map((svc, si) => (
              <ServiceBlock
                key={svc.id}
                svc={svc}
                serviceOptions={serviceOptions}
