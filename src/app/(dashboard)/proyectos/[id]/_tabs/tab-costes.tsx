...
import { MilestoneDetailDrawer } from "@/features/billing-milestones/components/milestone-detail-drawer"
...
type Props = { projectId: string; clientId: string; budget: number | null; currencyCode: string | null; organizationId: string }
...
export async function TabCostes({ projectId, clientId, budget, currencyCode, organizationId }: Props) {
  ...
  // Split expenses: linked to a milestone vs. general
  ...
  const hasRecurrentes = subscriptions.length > 0 || services.length > 0 || domains.some(d => d.renewal_price != null) || hosting.some(h => h.renewal_price != null)
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat
          ...
          subtitle="subscripciones y renovaciones"
        />
        <Stat
      ...
      <div className="flex flex-col gap-3">
        <SectionSeparator label="Cobros al cliente" />
                ...
                {milestones.length > 1 && (
                  <tfoot>
                    <tr className="border-t bg-muted/30">
                      <td colSpan={3} className="py-2.5 pl-4 text-xs text-muted-foreground">Total facturado</td>
          ...
          <details className="group rounded-xl border bg-card overflow-hidden">
            <summary className="flex cursor-pointer select-none items-center gap-2 px-4 py-3 text-sm font-medium hover:bg-muted/40 transition-colors [&::-webkit-details-marker]:hidden">
              ...
              {totalAnnualRevenue > 0 && (
                <span className="ml-1 text-xs font-normal text-muted-foreground">
                  {fmt(totalAnnualRevenue, cur)}/año
                </span>
              )}
              <span className="ml-auto text-xs text-muted-foreground font-normal">
                {subscriptions.length + services.length + domains.filter(d => d.renewal_price != null).length + hosting.filter(h => h.renewal_price != null).length} elementos
              </span>
            </summary>
            ...
            <div className="border-t flex flex-col divide-y">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b text-xs text-muted-foreground">
                          ...
                          <th className="pb-2 text-left font-medium">Próxima renovación</th>
                        ...
                        {subscriptions.map((s) => {
                          ...
                          return (
                            <tr key={s.id} className="border-b last:border-0">
                              <td className="py-2.5 font-medium text-foreground">{svcName}</td>
                              <td className="py-2.5">
                              ...
                              <td className="py-2.5 text-right font-mono text-muted-foreground">{fmt(annual, cur)}</td>
                            </tr>
                          )
                        })}
                      </tbody>
                      {subscriptions.length > 1 && (
                        <tfoot>
                          <tr>
                            <td colSpan={5} className="pt-3 text-xs text-muted-foreground">Total anual estimado</td>
                            <td className="pt-3 text-right font-mono font-semibold">{fmt(subscriptionsAnnual, cur)}</td>
                          </tr>
                        </tfoot>
                      )}
                    </table>
              ...
              <div className="px-4 py-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                    Servicios contratados ({services.length})
                  </p>
                  <CostAddService clientId={clientId} projectId={projectId} serviceOptions={serviceOptions} />
                </div>
                {services.length === 0 ? (
                  <p className="text-sm text-muted-foreground">Sin servicios. Añade servicios desde la ficha del cliente y asígnalos a este proyecto.</p>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        ...
                        {services.map((s) => {
                          ...
                          return (
                            <tr key={s.id} className="border-b last:border-0 group">
                              <td className="py-2.5 font-medium text-foreground">{serviceName}</td>
                      ...
                      {services.length > 1 && (
                        <tfoot>
                          <tr>
                            <td colSpan={3} className="pt-3 text-xs text-muted-foreground">Total anual estimado</td>
                            <td className="pt-3 text-right font-mono font-semibold">{fmt(servicesAnnual, cur)}</td>
                            <td />
                          </tr>
                        </tfoot>
                      )}
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {domains.filter(d => d.renewal_price != null).map((d) => (
                          <tr key={d.id} className="border-b last:border-0">
                            ...
                            <td className="py-2.5 text-right font-mono">{fmt(d.renewal_price, d.currency_code ?? cur)}</td>
                          </tr>
                        ))}
                        {hosting.filter(h => h.renewal_price != null).map((h) => (
                          <tr key={h.id} className="border-b last:border-0">
                            ...
                            <td className="py-2.5 text-right font-mono">{fmt(h.renewal_price, h.currency_code ?? cur)}</td>
                          </tr>
                        ))}
                      </tbody>
                      {(domainsRenewalAnnual + hostingRenewalAnnual) > 0 && (
                        <tfoot>
                          <tr>
                            <td colSpan={4} className="pt-3 text-xs text-muted-foreground">Total anual estimado</td>
                            <td className="pt-3 text-right font-mono font-semibold">{fmt(domainsRenewalAnnual + hostingRenewalAnnual, cur)}</td>
                          </tr>
                        </tfoot>
                      )}
                    </table>
      ...
      {/* ───── COSTES DEL PROYECTO ───── */}
      <div className="flex flex-col gap-3">
        <SectionSeparator label="Costes del proyecto" />
        ...
        {/* Costes de proveedores (colapsable) */}
        {hasInternalCosts && (
          <details className="group rounded-xl border bg-card overflow-hidden">
            <summary className="flex cursor-pointer select-none items-center gap-2 px-4 py-3 text-sm font-medium hover:bg-muted/40 transition-colors [&::-webkit-details-marker]:hidden">
              <ChevronRightIcon className="size-4 text-muted-foreground transition-transform group-open:rotate-90" />
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan={5} className="pt-3 text-xs text-muted-foreground">Total anual estimado</td>
                      <td className="pt-3 text-right font-mono font-semibold">{fmt(domainsInternalAnnual + hostingInternalAnnual, cur)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
